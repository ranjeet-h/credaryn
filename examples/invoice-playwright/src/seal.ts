...
import { createPlaywrightInvoiceRenderer, createPdfPipeline } from "@credaryn/pdf";
import { LocalSigner } from "@credaryn/provider-local";

const outputUrl = new URL("../../../artifacts/invoice-playwright/", import.meta.url);
...
export async function generate(): Promise<void> {
  const paperSigner = new LocalSigner({ issuerId: descriptor.issuerId, keyId: "playwright-paper-key" });
...
function placeSeal(pdfBytes: Uint8Array, transport: string): Uint8Array {
  if (pdfBytes[0] !== 0x25 || pdfBytes[1] !== 0x50 || pdfBytes[2] !== 0x44 || pdfBytes[3] !== 0x46) throw new Error("Playwright renderer did not produce a PDF");
