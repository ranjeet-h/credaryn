...
Complete Product Specification and Architecture

Status: Product definition and architecture; implementation status is tracked in the repository.
...
## 9. V2 \- Production Integrations and Interoperability
...
Browser verification: manual, using the browser-print example and the verifier PWA.
...
Continuous integration: none; the only workflow is the GitHub Pages documentation deployment.
...
Every PDF fixture published by Credaryn must be validated by DSS. A second independent PAdES-capable validator should be added before 1.0 to reduce implementation monoculture.
...
Test-first development is mandatory for every behavior added to the core, paper, PDF and verifier packages: write a failing test, then implement the behavior.
...
PDF validation gate: every generated fixture validates through DSS 6.5 at the claimed PAdES level.

Paper round-trip gate: encode \-\> render \-\> decode \-\> verify across multiple QR sizes and representative claim payloads.

Browser verification is manual; no Playwright or browser-automation suite is part of the repository.
...
This section defines the enterprise-production scope. V2 is not a rewrite of V1; every V2 component must preserve the V1 trust model, paper profile and normalized verification result.
...
PKCS\#11: production HSM access is isolated behind the SignerProvider boundary and tested against SoftHSM2. Do not make a low-level Node PKCS\#11 package part of Credaryn core. Vendor HSM certification is documented per supported device before it is listed as production-compatible.
...
Cloud-provider integration tests use provider emulators/mocks where available plus scheduled real-provider smoke tests without storing long-lived credentials in the repository.
...
## 21. Final Product Definition and Completion Criteria

This section defines the destination. Once these criteria are met, Credaryn is feature-complete for the defined product; subsequent work is normal product evolution.
...
Dependency/SBOM/release provenance and signed-release processes defined and exercised for every release.

Fuzz/property/mutation/adversarial parser tests run as part of the release test suite.
...
Credaryn is complete for the defined product vision when all of the following are true:
...
Post-completion backlog \- not required for completion
...
PAdES Baseline B-B is the V1 digital PDF profile; DSS 6.5 is the production/reference engine and validator, with B-T when an RFC 3161 TSA is configured.
