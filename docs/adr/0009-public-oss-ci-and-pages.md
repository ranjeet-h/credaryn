...
- Status: accepted
- Date: 2026-09-18
- Related: ADR 0004 (no CI; local and owner-run gates), ADR 0005 (manual UI verification)

## Context

Credaryn is being released as **public open source** under Apache-2.0. The project deliberately runs
its verification gates locally and by the owner (ADR 0004) and does not operate continuous
...
1. **No continuous integration.** There is no build/test CI and no CodeQL workflow. Verification
   stays local and owner-run exactly as described in ADR 0004; contributors run the gates documented
...
- Release provenance, signed tags and npm trusted publishing remain owner-run; this ADR does not
  claim them.
- Because there is no PR pipeline, reviewers rely on the documented local gates plus the committed
